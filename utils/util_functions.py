...
def get_average_cost(user, symbol, price):
    ...
    portfolio_mv = positions.aggregate(portfolio_mv=Sum(F("size")*F('price'),output_field=FloatField()))

    symbol_positions = positions.filter(stockname__symbol=symbol)

    total_amount, total_units, total_return, market_value = 0.0, 0.0, 0.0, 0.0
    ...
    for position in symbol_positions:
        market_value += (int(position.size) * float(price))
    ...
    total_return = (price - avg_cost) * total_units
    total_amount = avg_cost * total_units
    total_return_percent = float(total_return / total_amount) * 100 if total_amount else 0.0
    portfolio_diversity = float(market_value / portfolio_mv['portfolio_mv'] ) * 100.0 if portfolio_mv['portfolio_mv'] else 0.0
