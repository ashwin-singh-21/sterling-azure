...
class RealizedGainLoss(models.Model):
    """
    Model to store the Realized Gain/Loss.
    """
    user = models.ForeignKey(User, related_name='user_realized_gl_rel',
                              on_delete=models.SET_NULL, null=True,
                              db_index=True,
                              blank=True)
    ticker = models.ForeignKey(StockNames, related_name='stock_realized_gl_rel',
                              on_delete=models.SET_NULL, null=True,
                              blank=True)
    realised_gainloss = models.CharField(default=0, max_length=100, null=True,
                              blank=True)
    type = models.CharField(default="SELL", max_length=100)
    buying_price = models.CharField(default=0, max_length=100, null=True,
                              blank=True)
    selling_price = models.CharField(default=0, max_length=100, null=True,
                              blank=True)
    no_of_shares_sold = models.CharField(default=0, max_length=100, null=True,
                              blank=True)
    date = models.DateTimeField(auto_now=True)

    def __str__(self):
        return str(self.user.first_name) + " >>> " + str(self.ticker.symbol)
